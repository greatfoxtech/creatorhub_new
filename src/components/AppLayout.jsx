...
const NavSection = ({ title, items, isCollapsed }) => (
  <div className="space-y-1">
    ...
    {items.map((item) => <NavItem key={item.title} item={item} isCollapsed={isCollapsed} />)}
  </div>
);

const AppSidebar = ({ isCollapsed, toggleSidebar }) => (
  <div className={`hidden md:flex flex-col bg-[#121726] border-r border-white/10 transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"}`}>
    <div className={`flex items-center justify-between h-16 px-4 border-b border-white/10 ${isCollapsed ? "justify-center" : ""}`}>
      {!isCollapsed && <Logo />}
      <Button variant="ghost" size="icon" onClick={toggleSidebar} className="text-gray-400 hover:text-white hover:bg-white/10">
        {isCollapsed ? <ChevronsRight /> : <ChevronsLeft />}
      </Button>
    </div>
    <nav className="flex-1 space-y-4 p-2 overflow-y-auto">
      {navSections.map((section) => (
        <NavSection key={section.title} title={section.title} items={section.items} isCollapsed={isCollapsed} />
      ))}
    </nav>
    <div className={`p-2 border-t border-white/10`}>
      <div className={`bg-gradient-to-br from-[#4368D9]/20 to-[#6E43D9]/20 rounded-lg ${isCollapsed ? "p-2" : "p-4"}`}>
  ...
  return (
    <header className="flex h-16 items-center justify-between gap-4 border-b border-white/10 bg-[#121726] px-4 md:px-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={toggleMobileSidebar} className="md:hidden text-gray-400 hover:text-white">
  ...
  return (
    <>
      ...
      <div className="min-h-screen w-full flex bg-[#0A0D14]">
        <AppSidebar isCollapsed={isSidebarCollapsed} toggleSidebar={() => setSidebarCollapsed(!isSidebarCollapsed)} />

        <Sheet open={isMobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
          <SheetContent side="left" className="bg-[#121726] border-r-0 p-0">
            <div className="flex flex-col h-full">
              <div className="flex items-center h-16 px-4 border-b border-white/10"><Logo /></div>
              <nav className="flex-1 space-y-4 p-2 overflow-y-auto">
                {navSections.map((section) => (
                  <NavSection key={section.title} title={section.title} items={section.items} isCollapsed={false} />
                ))}
              </nav>
              <div className="p-2 border-t border-white/10">
                <div className="bg-gradient-to-br from-[#4368D9]/20 to-[#6E43D9]/20 p-4 rounded-lg">
                  ...
                  <Button size="sm" className="w-full bg-white text-slate-900 hover:bg-gray-200">Upgrade Now</Button>
                </div>
              </div>
            </div>
          </SheetContent>
